...
if __name__ == '__main__':
    ...
    if shutil.which( 'kissat' ) is None:
        print( 'kissat not found on PATH; install a SAT solver to rerun this historical differential-effect search.' )
        raise SystemExit(0)
    ...
    PATH = []
    count = 0
    res = CIPHER.diffEffectModel( start, start + ROUND, obj, INX, OUTX, PATH, seed = 15, time = 30 * 24 * 3600 )
    ...
    while isinstance( res, tuple ) and res[0] not in [0,-1]:
        ...
        print( math.log(T[a], 2) )

        PATH.append( res[1] )

        res = CIPHER.diffEffectModel( start, start + ROUND, obj + 10, INX, OUTX, PATH, seed = 15, time = 30 * 24 * 3600 )
        #res = CIPHER.diffEffectModel( start, start + ROUND, obj, INX, OUTX, PATH, seed = 15, time = 30 * 24 * 3600 )
