...
class boomerang:
    ...
    def boomerangAttack( self, start, end, middle, obj, seed = 0, time = 30 * 24 * 3600 ):
        '''
        ...
        for r in range( ROUND ):
            if r == middle:
                for i in range(16):
                    ...
                    csbox.print2DTable( csbox.genBCT(), row = 4, col = 4 )
                    print()
                    constr = csbox.gen_cnf_for_BCT( BIT = 4 )
                    L = []
                    L += X[r][4*i: 4*i+4]
                    L += Y[r][4*i: 4*i+4]
                    L += [ P0[r][i] ]
    ...
    def boomerangKeyRecovery( self, start, r0, r1, r2, middle, obj, pro, seed = 0, time = 30 * 24 * 3600 ):
        partcipher = []
        ...
        model.exclude_vector( Y[r0 + r1 - 1], [0 for i in range(64) ] )
